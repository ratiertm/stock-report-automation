...
class ZacksParser:
    ...
    def _parse_industry_comparison(self, pages_text: List[str], pdf, result: ZacksParseResult):
        ...
        # e.g., "3M Company (MMM) Neutral" — name can start with a digit
        # These appear after "Top Peers" header, before Industry Comparison table
        peer_pattern = r'([A-Za-z0-9][A-Za-z0-9\s&.,\'-]+?)\s+\((\w+)\)\s+(Neutral|Outperform|Underperform|Strong Buy|Strong Sell|Buy|Sell|Hold)'
        for m in re.finditer(peer_pattern, full_text):
            ...
            # Skip table header match: "Company (Ticker) Rec Rank" → ticker="Ticker"
            if ticker == "Ticker":
                continue

            # Clean peer_name: remove header residue like "Rec Rank\n" prefix
            name = re.sub(r'^(?:Rec Rank\s*\n?\s*|Company\s*\n?\s*)', '', name).strip()
